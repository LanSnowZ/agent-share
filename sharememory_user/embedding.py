...
class Embedder:
        ...
        if cfg.embed_use_hf:
            try:
                ...
                self.model = BGEM3FlagModel(
                    cfg.embed_model_name, use_fp16=cfg.embed_use_fp16
                )
                self._use_hf = True
                print("BGEM3FlagModel loaded successfully.")
            except ImportError:
                warnings.warn(
                    "'FlagEmbedding' package not found. Falling back to heuristic hashing."
                )
                self._use_hf = False
            except Exception as e:
                warnings.warn(
                    f"Failed to load BGEM3FlagModel: {e}. Falling back to heuristic hashing."
                )
                self._use_hf = False
    ...
    def embed_many(self, texts: Iterable[str]) -> List[List[float]]:
        if self._use_hf and self.model:
            ...
            if not text_list:
                return []
            dense_vecs = self.model.encode(text_list, batch_size=12, max_length=512)[
                "dense_vecs"
            ]
