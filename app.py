...
# 导入评估提示词
from eval.evaluation_prompts import get_fusion_rag_prompt, get_rag_answer_prompt, get_baseline_answer_prompt
...
def get_ingest_pipeline():
    ...
    if ingest_pipeline is None:
        ...
        # 导入时确保作为包导入
        from sharememory_user.pipeline_ingest import IngestPipeline as SharedIngestPipeline
...
def save_chain_to_shared_memory(user_id: str, chain_pages: List[Dict]):
    ...
    try:
        ...
        if memory_item:
            print(f"✅ 成功将思维链存储到共享记忆，Memory ID: {memory_item.id}, 对话轮数: {len(chain_pages)}")
        else:
            print(f"⚠️ 思维链未通过质量检查，未存储到共享记忆")
    except Exception as e:
        print(f"❌ 存储思维链到共享记忆失败: {e}")

def check_and_store_chain_break(user_id: str, new_user_msg: str, new_agent_msg: str) -> None:
    """检测思维链断裂并存储"""
...
def ensure_user_memoryos(user_id: str, project_name: str = "default_project") -> Optional[Memoryos]:
...
def get_user_config(user_id: str, project_name: str = "default_project") -> Dict[str, Any]:
    """获取用户配置"""
    config_path = os.path.join(MEMORYOS_DATA_DIR, project_name, "users", user_id, "config.json")
    if os.path.exists(config_path):
        with open(config_path, 'r', encoding='utf-8') as f:
            return json.load(f)
    return {}

def save_user_config(user_id: str, config_data: Dict[str, Any], project_name: str = "default_project") -> bool:
    """保存用户配置"""
    try:
...
def get_chat_conversations(user_id: str, project_name: str = "default_project") -> List[Dict[str, Any]]:
    """获取用户的聊天对话列表"""
    conversations_path = os.path.join(MEMORYOS_DATA_DIR, project_name, "users", user_id, "conversations.json")
    if os.path.exists(conversations_path):
        with open(conversations_path, 'r', encoding='utf-8') as f:
            return json.load(f)
    return []

def save_chat_conversations(user_id: str, conversations: List[Dict[str, Any]], project_name: str = "default_project") -> bool:
    """保存用户的聊天对话列表"""
    try:
        user_dir = os.path.join(MEMORYOS_DATA_DIR, project_name, "users", user_id)
        os.makedirs(user_dir, exist_ok=True)
...
def get_chat_messages(user_id: str, conversation_id: str, project_name: str = "default_project") -> Optional[Dict[str, Any]]:
    """获取指定对话的消息"""
    conversation_path = os.path.join(MEMORYOS_DATA_DIR, project_name, "users", user_id, "conversations", f"{conversation_id}.json")
...
def save_chat_conversation(username, conversation_id, user_message, ai_response, model, shared_memory_enabled=False, personal_memory_enabled=True, update_last_ai_message=False, user_message_only=False):
    ...
    # 创建用户目录
    user_chat_dir = os.path.join(MEMORYOS_DATA_DIR, "default_project", "users", username)
    ...
    if not conversation_data.get('title'):
        # 使用用户的第一条消息作为标题
        first_user_message = next((msg['content'] for msg in conversation_data['messages'] if msg['type'] == 'user'), '新对话')
        conversation_data['title'] = first_user_message[:30] + ('...' if len(first_user_message) > 30 else '')
...
def load_conversation_history(username, conversation_id):
    """加载对话历史"""
    try:
        conversation_file = os.path.join(MEMORYOS_DATA_DIR, "default_project", "users", username, f"{conversation_id}.json")
        
        if os.path.exists(conversation_file):
            with open(conversation_file, 'r', encoding='utf-8') as f:
                conversation_data = json.load(f)
            print(f"✅ 加载对话历史成功，消息数: {len(conversation_data.get('messages', []))}")
...
def get_baseline_answer_prompt_no_profile(user_query: str, conversation_context: str = "") -> str:
    """
...
def generate_response_without_memory(user_id: str, message: str, model: str, project_name: str = "default_project", conversation_id: str = None) -> str:
    """
    ...
    try:
        ...
        if conversation_id:
            ...
            if conversation_data and conversation_data.get('messages'):
                # 获取历史消息（排除当前消息）
                history_messages = conversation_data['messages'][:-2] if len(conversation_data['messages']) >= 2 else []
                if history_messages:
                    conversation_context = "\n".join([
...
def get_rag_answer_prompt_with_context(user_query: str, retrieved_context: str, user_profile: str, conversation_context: str = "") -> str:
    """
    创建包含对话上下文的RAG提示词
    """
    context_section = ""
...
def get_fusion_rag_prompt_with_context(user_query: str, shared_memory_context: str, personal_memory_context: str, user_profile: str, conversation_context: str = "") -> str:
    """
    创建包含对话上下文的融合RAG提示词
    """
    context_section = ""
...
def generate_response_with_memory(user_id: str, message: str, model: str, shared_memory_enabled: bool = False, personal_memory_enabled: bool = True, project_name: str = "default_project", conversation_id: str = None) -> str:
    ...
    try:
        ...
        if personal_memory_enabled and user_id in memoryos_instances:
            try:
                ...
                # 获取长期用户档案
                long_term_profile = memoryos_instance.user_long_term_memory.get_raw_user_profile(user_id)
                ...
                personal_memory_context = format_memoryos_retrieval_result(context_result)
        ...
        if shared_memory_enabled:
            try:
                ...
                if retrieval_result['items']:
                    shared_memory_context = retrieve_pipeline.build_prompt_blocks(retrieval_result['items'])
                
                print(f"🔗 Retrieved shared memory context: {len(shared_memory_context)} chars")
                
            except Exception as e:
                print(f"检索共享记忆失败: {e}")
        
        # 根据记忆状态选择提示词 (与原始项目逻辑一致)
        if personal_memory_enabled and shared_memory_enabled and shared_memory_context and personal_memory_context:
...
def get_memory_file():
    ...
    try:
        ...
        file_path = os.path.join(MEMORYOS_DATA_DIR, username, "users", username, file_name)
...
def chat_direct():
    ...
    def generate():
        try:
            ...
            print(f"🔘 个人记忆: {personal_memory_enabled}, 共享记忆: {shared_memory_enabled}")
            ...
            if personal_memory_enabled and username in memoryos_instances:
                try:
                    ...
                    long_term_profile = memoryos_instance.user_long_term_memory.get_raw_user_profile(username)
                    if long_term_profile and long_term_profile != "None":
            ...
            if shared_memory_enabled:
                try:
                    peers = retrieve_pipeline.get_cached_peers()
                    enhanced_user_profile = UserProfile(user_id=username, profile_text=enhanced_profile_text)
                    retrieval_result = retrieve_pipeline.retrieve(
                        user=enhanced_user_profile,
                        task=message,
                        peers=peers,
            ...
            if personal_memory_enabled and shared_memory_enabled and shared_memory_context and personal_memory_context:
                prompt = get_fusion_rag_prompt_with_context(
                    message, shared_memory_context, personal_memory_context,
                    enhanced_profile_text, conversation_context
                )
                print("🧠 使用融合RAG提示词")
            elif personal_memory_enabled and personal_memory_context:
                prompt = get_fusion_rag_prompt_with_context(
                    message, "", personal_memory_context,
                    enhanced_profile_text, conversation_context
                )
                print("🧠 使用个人记忆RAG提示词")
            elif shared_memory_enabled and shared_memory_context:
                prompt = get_rag_answer_prompt_with_context(
                    message, shared_memory_context,
                    enhanced_profile_text, conversation_context
                )
                print("🔗 使用共享记忆RAG提示词")
            else:
