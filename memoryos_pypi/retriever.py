...
try:
    ...
    from .utils import OpenAIClient, get_timestamp, run_parallel_tasks
except ImportError:
    from long_term import LongTermMemory
    from mid_term import MidTermMemory
...
class Retriever:
    def __init__(
        ...
        assistant_long_term_memory: Optional[
            LongTermMemory
        ] = None,  # Add assistant LTM
    ...
    def _retrieve_mid_term_context(
        self,
        user_query,
    ...
    def _retrieve_user_knowledge(
        self, user_query, knowledge_threshold, top_k_knowledge
    ):
        """并行任务：从用户长期知识检索"""
        ...
        print(
            f"Retriever: Long-term user knowledge recalled {len(retrieved_knowledge)} items."
        )
        return retrieved_knowledge

    def _retrieve_assistant_knowledge(
        self, user_query, knowledge_threshold, top_k_knowledge
    ):
        """并行任务：从助手长期知识检索"""
        ...
        print(
            f"Retriever: Long-term assistant knowledge recalled {len(retrieved_knowledge)} items."
        )
        return retrieved_knowledge
    ...
    def retrieve_context(
        ...
        top_k_knowledge=1,  # Default for knowledge search
    ):
        print(
            f"Retriever: Starting PARALLEL retrieval for query: '{user_query[:50]}...'"
        )

        # 并行执行三个检索任务
        tasks = [
            lambda: self._retrieve_mid_term_context(
                user_query,
                segment_similarity_threshold,
                page_similarity_threshold,
                top_k_sessions,
            ),
            lambda: self._retrieve_user_knowledge(
                user_query, knowledge_threshold, top_k_knowledge
            ),
            lambda: self._retrieve_assistant_knowledge(
                user_query, knowledge_threshold, top_k_knowledge
            ),
        ]
        ...
        return {
            ...
            or [],  # List of knowledge entry dicts
            "retrieved_assistant_knowledge": retrieved_assistant_knowledge
            or [],  # List of assistant knowledge entry dicts
