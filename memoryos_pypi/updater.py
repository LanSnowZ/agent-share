...
class Updater:
    ...
    def _process_page_embedding_and_keywords(self, page_data):
        ...
        if "page_embedding" in page_data and page_data["page_embedding"]:
            print(
                f"Updater: Page {page_id} already has embedding, skipping computation"
            )
            return page_data
    ...
    def process_short_term_to_mid_term(self):
        ...
        for qa_pair in evicted_qas:
            ...
            current_page_obj = {
                "page_id": qa_pair.get("page_id")
                or generate_id("page"),  # Preserve existing ID
            ...
            if not current_page_obj.get("meta_info"):
                if current_page_obj.get("pre_page"):
                    ...
                    last_meta = (
                        temp_last_page_in_batch.get("meta_info")
                        if temp_last_page_in_batch
                        else None
                    )
    ...
    def update_long_term_from_analysis(self, user_id, profile_analysis_result):
        ...
        if new_profile_text and new_profile_text.lower() != "none":
            ...
            self.long_term_memory.update_user_profile(
                user_id, new_profile_text, merge=False
            )
