...
class Memoryos:
    def __init__(
        ...
        if embedding_model_kwargs is None:
            if "bge-m3" in self.embedding_model_name.lower():
                print(
                    "INFO: Detected bge-m3 model, defaulting embedding_model_kwargs to {'use_fp16': True}"
                )
        ...
        print(
            f"Using embedding model: {self.embedding_model_name} with kwargs: {self.embedding_model_kwargs}"
        )
        ...
        if current_heat >= self.mid_term_heat_threshold:
            ...
            if unanalyzed_pages:
                ...
                def task_user_profile_analysis():
                    print(
                        "Memoryos: Starting parallel user profile analysis and update..."
                    )
                    # 获取现有用户画像
                    existing_profile = self.user_long_term_memory.get_raw_user_profile(
                        self.user_id
                    )
                    ...
                    # 直接输出更新后的完整画像
                    return gpt_user_profile_analysis(
                        unanalyzed_pages,
                        self.client,
    ...
    def add_memory(
        ...
        print(
            f"Memoryos: Added QA to short-term. User: {user_input[:30]}... (page_id={page_id}, pre_page={pre_page_id})"
        )
    ...
    def get_response(
        ...
        self.add_memory(
            user_input=query, agent_response=response_content, timestamp=get_timestamp()
        )
