...
class MidTermMemory:
    def __init__(
        ...
        self.embedding_model_kwargs = (
            embedding_model_kwargs if embedding_model_kwargs is not None else {}
        )
        self.load()
    ...
    def add_session(self, summary, details, summary_keywords=None):
        ...
        summary_vec = get_embedding(
            summary, model_name=self.embedding_model_name, **self.embedding_model_kwargs
        )
        ...
        for page_data in details:
            ...
            if "page_embedding" in page_data and page_data["page_embedding"]:
                ...
                if isinstance(inp_vec, list):
                    ...
                    if (
                        np.linalg.norm(inp_vec_np) > 1.1
                        or np.linalg.norm(inp_vec_np) < 0.9
                    ):  # 检查是否需要重新normalize
            ...
            if "page_keywords" in page_data and page_data["page_keywords"]:
                ...
                print(
                    f"MidTermMemory: Setting empty keywords for page {page_id} (will be filled by multi-summary)"
                )
                page_keywords = []
    ...
    def insert_pages_into_session(
        ...
        if not self.sessions:  # If no existing sessions, just add as a new one
            ...
            return self.add_session(
                summary_for_new_pages, pages_to_insert, keywords_for_new_pages
            )
        ...
        if best_sid and best_overall_score >= similarity_threshold:
            ...
            for page_data in pages_to_insert:
                ...
                if "page_embedding" in page_data and page_data["page_embedding"]:
                    print(
                        f"MidTermMemory: Reusing existing embedding for page {page_id}"
                    )
                    inp_vec = page_data["page_embedding"]
                    # 确保embedding是normalized的
                    if isinstance(inp_vec, list):
    ...
    def search_sessions(
        ...
        query_keywords = (
            set()
        )  # Keywords extraction removed, relying on semantic similarity
